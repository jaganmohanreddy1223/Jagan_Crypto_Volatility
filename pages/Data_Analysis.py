...
st.subheader("📌 Key Risk Metrics (Per Crypto)")
...
# ==========================================================
# ✅ NEW: OVERALL / AVERAGE KEY METRICS
# ==========================================================
st.subheader("📊 Overall Average Metrics (Selected Cryptos)")
filtered_data = pd.concat(
    [
        df.assign(Crypto=crypto)
        for crypto, df in data.items()
    ]
)
filtered_data["Sharpe_Ratio"] = (
    filtered_data["Return"] / filtered_data["Volatility"]
) * np.sqrt(252)

col1, col2, col3 = st.columns(3)
col1.metric(
    "Avg Volatility",
    round(filtered_data["Volatility"].mean(), 4)
)

col2.metric(
    "Avg Return",
    round(filtered_data["Return"].mean(), 4)
)
col3.metric(
    "Avg Sharpe Ratio",
    round(filtered_data["Sharpe_Ratio"].mean(), 2)
)
# ------------------ RISK vs RETURN ------------------
